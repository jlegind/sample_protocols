...
import requests
import pandas as pd
import search_gbif_metadata as search_metadata
import os

'''Package shall return probably candidate datasets for term entered into the main search method. This is part of the GBIF dataset tagging effort.'''
...
dct = {}
dct['number'] = 0
d1_test_json =  {"samplingDescription": {
    ...
    for f in fields:
        ...
        for j in res['results']:
            ...
            # for f in fields:
                ...
                if x :
                    dataset_title = search_metadata.field_lookup(datasetkey, 'title')
                    publisher_key = search_metadata.field_lookup(datasetkey, 'publishingOrganizationKey')
                    publisher = search_metadata.field_lookup(publisher_key, 'title', 'http://api.gbif.org/v1/organization/')
                    x.append(lookup)
                    x.append(dataset_title)
                    x.append(publisher)
                    dflist.append(x)
...
cols = ['term', 'target-word', 'distance', 'json field', 'datasetkey', 'context', 'dataset_title', 'publisher']
fin = list()
print('dflist>> ', dflist)
# for l in dflist:
